...
def solution(fname, scrambled):
    with open(fname, 'r') as f:
        content = f.read().strip().split('\n')
    inp = scrambled
    for permutation in permutations(inp):
        inp = [char for char in "".join(permutation)]
        new_string = inp
        for line in content:
            args = line.split(' ')
            if args[0] == "rotate":
            ...
            inp = new_string   
            #print(line, "->>", "".join(inp))
        if "".join(inp) == scrambled:
            return "".join(permutation)
    return -1

if __name__ == "__main__":
    print(solution('input.txt', "fbgdceah"))