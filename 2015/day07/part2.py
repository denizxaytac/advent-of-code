...
def main(p_input):
    ...
    result = evaluate(copy.deepcopy(values), copy.deepcopy(operations))['a']

    for idx, operation in enumerate(operations):
        value , wire = operation.split(" -> ")
        if wire == 'b':
            operations[idx] = str(result) + " -> b"

    new_result = evaluate(copy.deepcopy(values), copy.deepcopy(operations))
    return new_result['a']
