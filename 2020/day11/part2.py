...
def change_state(grid):
    ...
    for idx_r, row in enumerate(grid):
        ...
        for idx_c, col in enumerate(row):
            ...
            for r_dir in range(-1, 2, 1):
                for c_dir in range(-1, 2, 1):
                    rr = r_dir
                    cc = c_dir
                    while True:
                        if rr == 0 and cc == 0:
                            break
                        if idx_r + rr < 0 or idx_r + rr > len(grid) - 1 or idx_c + cc < 0 or idx_c + cc > len(grid[0]) - 1:
                            break
            ...
            if col == ".":
                ...
                new_row.append("#")
            elif col == "#" and neighbours["occupied"] >= 5:
                new_row.append("L")
            else:
